...
public class OpenMaze {
    ...
    public void solveGraphBFS() {
        ...
        boolean[][] seen = new boolean[this.n+1][this.n+1];
        ...
        seen[1][1] = true;
        while (queue.size() > 0 && !this.done) {
            Point curr = queue.remove();
            if (curr.isEqual(this.end)) {
                this.done = true;
            }
            selectPoint(curr);
            Point[] children = this.getMoves(curr);
            for (int i = 0; i < children.length; i++) {
                if (!seen[children[i].x][children[i].y]) {
                    queue.add(children[i]);
                    seen[children[i].x][children[i].y] = true;
                }
            }
        }
    }
