...
public class Maze {
    ...
    public Point[] getChildren(Point point) {
        ...
        Point[] childrenFinal = new Point[children.size()];
        for (int i=0; i < children.size(); i++) {
            childrenFinal[i] = children.get(i);
        }
     ...
     */
    /*private void solveDFSRecursive(Point point, Boolean[][] memo) {
        if (!memo[point.x][point.y] && !done) {
            ...
            if (point.isEqual(end)) {
                ...
                for (Point child : children) {
                    //if (!memo[child.x][child.y]) {
                    if (!done) {
                        selectPoint(child);
                        solveDFSRecursive(child, memo);
                    }
                    //}
                }
            }
    ...
    private void solveDFSRecursive(Point point) {
            ...
            if (point.isEqual(end)) {
                ...
                for (Point child : children) {
                    //if (!memo[child.x][child.y]) {
                    if (!done && !child.isEqual(point.parent)) {
